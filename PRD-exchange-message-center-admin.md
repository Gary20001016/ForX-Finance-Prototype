...
> 更新日期：2026-07-13
>
> 产品范围：Web 用户端消息中心 + 消息运营后台 + 基础数据分析
>
> 当前交付：前端交互原型；后端接口、真实数据与 App Push 接入不在本轮开发范围
>
> 文档状态：已完成范围收敛，替代 V1.0
...
第一期以 Web 站内信为主，预留 App Push。交易所必须对强平预警、提现风险、账户异常等重要消息提供明显提示，并保留发送记录、审批记录和操作日志。
...
- 第一期用户渠道仅为 Web 站内信。
- App Push 只预留模板字段、设备和埋点协议，不做真实推送。
...
| 紧急 | 红色标识、置顶 | 红色警示区 | 需要明确确认，可预留 Push |
...
模板必须支持预览、变量示例值、缺失变量提示和语言回退。回退顺序为用户语言 → 同语言通用版本 → 模板默认语言；紧急消息缺少可用语言时不得静默发送。
...
- 第一期启用 Web 站内信。
- App Push 预留标题、正文、设备平台、Push Token、点击动作和回执字段。
- 邮件、短信不进入第一期导航和验收。
- 后台数据模型保留 `channel` 字段，第一期默认值为 `inbox`。
...
第一期角色：内容编辑、运营人员、业务审核、风控审核、管理员、审计员。关键日志记录操作人、对象、变更前后内容、时间、IP/设备摘要和结果。

必须审计：模板发布、全站发送、紧急消息发送、审批、全部已读接口异常、分类修改、链接白名单修改和数据导出。
...
- 风险消息 5 分钟、30 分钟阅读率。
- 过期仍未读的风险消息数量。
- Web 与 App 对比位置预留，第一期 App 显示“未接入”。
...
8. Web 站内信完整可用，App Push 字段和分析位置已预留。
...
V2.0 的核心不是继续扩展后台功能，而是补齐用户真正使用的消息中心，并让后台任务、模板、事件、风险治理、发送记录和数据分析形成最小闭环。第一期优先把 Web 站内信体验和管理流程做完整，其他渠道与高级营销能力按发布计划后置。
