...
export default function MessageDetailPage(){
  ...
  const openAction=()=>{if(isExpired){Message.warning('消息已过有效期，业务动作不可继续');return;}if(message.risk==='关键'&&!message.acknowledgedAt){Modal.confirm({title:'确认已知悉风险',content:'继续表示您已阅读风险提示。请及时处理仓位或账户异常。',okText:'已知悉并继续',onOk:()=>{acknowledgeRiskMessage(message.id);jump();}});return;}jump();};
  return <main className="inbox-shell"><header className="inbox-topbar"><div className="inbox-brand"><span>F</span><strong>ForX Finance</strong></div><Button type="text" onClick={()=>navigate('/dashboard')}>运营后台</Button></header><article className={`message-detail detail-risk-${message.risk}`}><Button type="text" icon={<IconArrowLeft/>} onClick={()=>navigate('/inbox')}>返回消息中心</Button>{message.risk==='关键'&&<Alert className="risk-alert" type="error" showIcon icon={<IconExclamationCircle/>} title="关键风险提示" content="请立即核对以下信息并及时处理。风险标签在消息已读后仍会保留。"/>}{message.risk==='高'&&<Alert className="risk-alert" type="warning" showIcon title="高风险消息" content="请及时核对账户与资产信息。"/>}<div className="detail-heading"><Space><Tag color={category.color}>{category.name}</Tag><Tag color={message.risk==='关键'?'red':message.risk==='高'?'orangered':message.risk==='中'?'orange':'gray'}>{message.risk}</Tag>{isExpired&&<Tag>已过期</Tag>}</Space><h1>{message.title}</h1><p>{message.createdAt} · {message.source} · <IconCheckCircle/> 已读</p></div><div className="detail-body"><p>{message.body}</p>{message.expiresAt&&<div className="detail-expiry">消息有效期至 {message.expiresAt} · {isExpired?'业务动作已关闭':'当前仍可处理'}</div>}</div>{message.actionText&&<Button type="primary" status={message.risk==='关键'?'danger':undefined} size="large" disabled={isExpired} onClick={openAction}>{isExpired?'已过期':message.actionText}</Button>}</article></main>;
}
