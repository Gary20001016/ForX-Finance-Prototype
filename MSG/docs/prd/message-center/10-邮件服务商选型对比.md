...
> 文档类型：技术与供应商选型参考  
> 更新日期：2026-08-18  
> 适用范围：面向全球用户的 Email 通知与运营发送
> 价格口径：供应商官网公开美元标价，不含税费、企业折扣及定制合同价格  
> 范围说明：Email 已纳入消息中心正式渠道；本文定义生产服务商与后台适配层选型，模块需求以本目录 00—09 文档为准。
...
flowchart LR
    ...
    B --> C{"发件人身份路由"}
    C -->|"关键通知身份"| D["Postmark Transactional Stream"]
    C -->|"批量运营身份"| E["Postmark Broadcast Stream"]
    D --> F["收件服务器"]
    E --> F
...
- 运营页面不设置邮件类型；发件人身份自动绑定发送流。开启退订配置时必须同时包含正文退订链接与标准退订 Header。
