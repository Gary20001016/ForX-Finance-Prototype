...
export default function SegmentListPage() {
  ...
  return <section className="page-stack">
    <PageHeader title="用户分群" description="按地区、资产、交易行为和消息偏好构建可审计受众。" actions={<WritePermissionButton type="primary" icon={<IconPlus />} onClick={() => openEditor('new')}>新建分群</WritePermissionButton>} />
    ...
    <Drawer width={560} visible={Boolean(selected)} title={selected ? `分群详情 · ${selected.name}` : '分群详情'} onCancel={() => setSelected(undefined)} footer={<WritePermissionButton type="primary" onClick={() => {if(selected){openEditor(selected);setSelected(undefined);}}}>编辑规则</WritePermissionButton>}>
