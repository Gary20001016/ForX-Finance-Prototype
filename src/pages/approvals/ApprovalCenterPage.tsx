...
export default function ApprovalCenterPage({ currentAdminId='admin-01' }: { currentAdminId?:string }) {
  ...
  const simulatedReviewerId=searchParams.get('simulateReviewer');
  const simulatedReviewer=store.operators.find((item)=>item.id===simulatedReviewerId);
  const effectiveAdminId=simulatedReviewer?.id||currentAdminId;
  ...
  const pendingApprovals=useMemo(()=>approvals.filter((item)=>item.assigneeId===effectiveAdminId&&['待我审核','待审核','紧急'].includes(item.status)),[approvals,effectiveAdminId]);
  const data=useMemo(()=>approvals.filter((item)=>(tab==='all'||tab==='mine'&&item.assigneeId===effectiveAdminId&&['待我审核','待审核','紧急'].includes(item.status)||tab==='emergency'&&item.assigneeId===effectiveAdminId&&item.emergency)&&`${item.id}${item.name}${item.submitter}${item.assignee||''}`.toLowerCase().includes(keyword.toLowerCase())&&(!filterOne||item.risk===filterOne)&&(!filterTwo||item.objectType===filterTwo)),[approvals,effectiveAdminId,tab,keyword,filterOne,filterTwo]);
  const approvalColumns:TableColumnProps<ApprovalItem>[]=[
    ...
    {title:'操作',fixed:'right',width:90,render:(_,item)=><Button type="text" onClick={()=>setSelected(item)}>{canWrite&&item.assigneeId===effectiveAdminId&&item.submitterId!==effectiveAdminId?'审核':'查看'}</Button>},
  ];

  return <section className="page-stack">
    <PageHeader title="审核中心" description="统一处理内容审核、任务配置审核和事件通知规则审核；新内容审核通过后才进入多语言。"/>
    {simulatedReviewer&&<Alert type="warning" showIcon content={`演示模式：当前以审核人「${simulatedReviewer.name}」身份处理工单；正式权限与禁止自审规则保持不变。`}/>}
    ...
    <ApprovalDrawer item={selected} visible={Boolean(selected)} onClose={()=>setSelected(undefined)} currentAdminId={effectiveAdminId} canWrite={canWrite}/>
  </section>;
}
