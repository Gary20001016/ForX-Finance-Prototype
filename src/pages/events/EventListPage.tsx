...
export default function EventListPage(){
  return <section className="page-stack"><PageHeader title="事件管理" description="管理业务事件契约、模板绑定、幂等与调用质量。" actions={<Button type="primary" icon={<IconPlus />} onClick={() => openDetailedForm('event','注册业务事件')}>注册事件</Button>}/><FilterBar><Input.Search placeholder="搜索事件编码或名称" style={{width:280}}/><Select placeholder="业务线" style={{width:140}}><Select.Option value="wallet">资金</Select.Option><Select.Option value="risk">安全</Select.Option></Select><Select placeholder="调用状态" style={{width:140}}><Select.Option value="healthy">运行正常</Select.Option><Select.Option value="delayed">轻微延迟</Select.Option></Select></FilterBar><ResourceTable data={events} rowKey="id" columns={[{title:'事件',width:280,render:(_,r)=><div><Typography.Text className="strong">{r.name}</Typography.Text><div className="mono muted">{r.id}</div></div>},{title:'业务线',dataIndex:'line',width:90},{title:'版本',dataIndex:'version',width:90},{title:'绑定模板',dataIndex:'template',width:180},{title:'调用方',dataIndex:'caller',width:170,render:v=><Tag>{v}</Tag>},{title:'近24h调用',dataIndex:'calls',width:110},{title:'失败率',dataIndex:'failure',width:90},{title:'最后调用',dataIndex:'last',width:120},{title:'状态',width:110,render:(_,r)=><StatusTag status={r.status}/>}]} /></section>;
}
