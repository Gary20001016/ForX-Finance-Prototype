...
export default function SettingsPage(){
  return <section className="page-stack">
    ...
    <Tabs type="card" defaultActiveTab="categories">
      <Tabs.TabPane key="categories" title="消息分类"><Card bordered={false} className="surface" title="分类字典" extra={<Button type="primary" size="small" icon={<IconPlus/>} onClick={() => openPrototypeDialog('新增消息分类','配置分类编码、多语言名称、默认性质、风险等级、渠道、退订和审批流程。')}>新增分类</Button>}><div className="settings-list">{categories.map(([name,nature,risk,optout])=><div key={name}><strong>{name}</strong><Tag>{nature}</Tag><span>默认风险：{risk}</span><span>{optout}</span><StatusTag status="可用"/><Button type="text" onClick={() => openPrototypeDialog(`编辑分类 · ${name}`,`当前性质：${nature}；默认风险：${risk}；退订规则：${optout}。`)}>编辑</Button></div>)}</div></Card></Tabs.TabPane>
